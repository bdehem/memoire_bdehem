...
#include "boris_drone/strategy_1.h"
...
Strategy::Strategy()
{
  ...
  strategy_pub = nh.advertise<std_msgs::Int16>(strategy_channel, 1);
...
void Strategy::publish_strategy()
{
  ...
  std_msgs::Int16 strategy_msg;

  strategy_msg.data = strategy;
...
while (ros::ok())
{
  ...
  if (strategy == TAKEOFF && altitude > 0.5)
  {
