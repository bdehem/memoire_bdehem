...
void PathPlanning::strategyCb(const boris_drone::StrategyMsg::ConstPtr strategyPtr)
{
  ...
  if (strategy == GOTO)
  {
    destination_x = strategyPtr->x;
    destination_y = strategyPtr->y;
    destination_z = strategyPtr->z;
  }
}
...
void PathPlanning::go_to()
{
  publish_poseref(destination_x,destination_y,destination_z,0);
}
