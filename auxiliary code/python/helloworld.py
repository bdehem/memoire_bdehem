...
thisfile = 'benchmark_basic2.bag'
thisfile = '/home/bor/bagfiles/benchmark_point_threshold/thresh_250.bag'
i = 0
w = 40
h = 3
ba_data = [[0 for x in range(w)] for y in range(h)]
for topic, msg, t in rosbag.Bag(thisfile).read_messages():
    if topic == "/benchmark":
...
#ba=rosbag_pandas.bag_to_dataframe('benchmark_basic2.bag',include="/benchmark")
...
vis = rosbag_pandas.bag_to_dataframe(thisfile,include="/pose_visual")
man = rosbag_pandas.bag_to_dataframe(thisfile,include="/pose_estimation")

plt.plot(vis.index, vis['pose_visual__y'])
plt.plot(man.index, man['pose_estimation__y'])
plt.show()
